...
st.set_page_config(
    page_title="Vendor Invoice Intelligence Platform",
    page_icon="📊",
    layout="wide"
)
...
/* ── Hero Banner ── */
.hero-banner {
    background: linear-gradient(120deg, #101014 0%, #121218 72%, #17121f 100%);
...
.section-icon {
    ...
    background: linear-gradient(135deg, rgba(167,139,250,0.12), rgba(99,102,241,0.12));
...
hr { border-color: rgba(167,139,250,0.07) !important; }
...
/* Purple-black refinement: restrained, editorial, enterprise */
.stApp {
    background: #08080b;
}
...
.section-header {
    border-bottom-color: rgba(167,139,250,.13);
}
.section-icon {
    box-shadow: inset 0 0 20px rgba(167,139,250,.035);
}
.stNumberInput input {
...
with st.sidebar:
    ...
    st.markdown(
        '<div style="font-size:10px;letter-spacing:2px;color:#46414d;font-family:\'DM Mono\',monospace;margin-bottom:8px;">BUSINESS IMPACT</div>',
        unsafe_allow_html=True
    )
    for item in [
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("""
    <div style="font-size:11px;color:#15151b;font-family:'DM Mono',monospace;line-height:2;">
        TECH STACK<br>
